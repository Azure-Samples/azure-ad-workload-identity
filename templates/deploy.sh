...
if [[ $installExtensions == 1 ]]; then
    ...
    aksExtensions=("PodSecurityPolicyPreview" "RunCommandPreview" "EnablePodIdentityPreview " "UserAssignedIdentityPreview" "EnablePrivateClusterPublicFQDN" "PodSubnetPreview" "EnableOIDCIssuerPreview")
    ok=0
...
echo "Checking if [$userPrincipalName] user has been assigned to [Azure Kubernetes Service Cluster Admin Role] role on the [$aksName] AKS cluster..."
role=$(az role assignment list \
    --assignee $userObjectId \
    --scope $aksClusterId \
    --query [?roleDefinitionName].roleDefinitionName \
...
if [[ $role == "Owner" ]] || [[ $role == "Contributor" ]] || [[ $role == "Azure Kubernetes Service Cluster Admin Role" ]]; then
    echo "[$userPrincipalName] user is already assigned to the [$role] role on the [$aksName] AKS cluster"
else
    echo "[$userPrincipalName] user is not assigned to the [Azure Kubernetes Service Cluster Admin Role] role on the [$aksName] AKS cluster"
    echo "Assigning the [$userPrincipalName] user to the [Azure Kubernetes Service Cluster Admin Role] role on the [$aksName] AKS cluster..."
    ...
    if [[ $? == 0 ]]; then
        echo "[$userPrincipalName] user successfully assigned to the [Azure Kubernetes Service Cluster Admin Role] role on the [$aksName] AKS cluster"
    else
        echo "Failed to assign the [$userPrincipalName] user to the [Azure Kubernetes Service Cluster Admin Role] role on the [$aksName] AKS cluster"
        exit
    fi
fi
